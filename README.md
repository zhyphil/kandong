...
6. 点显示窗右上方的收起图标，镜面与取景框会收为一个贴边放大镜按钮。可拖动按钮换位置，轻点恢复原选区、倍率和镜面滑动位置；长按按钮打开菜单。收起或打开菜单时暂停画面处理，但本次系统共享会话仍保留。
7. 菜单提供使用帮助与隐私说明；翻译设置、账号、订阅标为暂不可用。本版无需登录。点方形结束图标，或菜单/首页的“结束共享”，关闭共享及所有悬浮窗口。**锁屏后再解锁，放大窗消失是正常行为**；需要时回到看懂再点“打开放大镜”，重新确认本次屏幕共享。旋转后同样停止。
