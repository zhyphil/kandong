...
5. 直接拖红色选框的内部或边框来移动选区；拖右上角的小箭头自由调整宽度和高度。放大画面中，双指张开/捏合调节1～5倍，倍率数字和滑杆同步更新；也可单独拖滑杆，每次新会话默认2倍。单指在放大画面内滑动查看其余内容。调红框大小不改变倍率；双指缩放不改变红框。取景框靠上，放大窗在下；取景框靠下，放大窗自动到上方。红框可直接拖到屏幕边缘，不需要额外松手贴边。
