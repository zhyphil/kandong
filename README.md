...
当前先做 **Phase 0A 放大镜技术验证**。不改变原 App 布局、字号或按钮位置。目标华为兼容版改为拖动取景框大小、自动显示实际倍率，显示窗随取景位置自动避让。翻译是下一阶段，AI 解释和下一步高亮随后推进；没有自动点击、输入、确认、提交、购买或支付。
...
5. 拖动取景框旁的移动手柄选位置，拖动箭头角柄等比例调整大小。向内拉，文字变大；向外拉，看更多内容。倍率自动显示，文字不变形。取景框靠上，放大窗在下；取景框靠下，放大窗自动到上方。框内仍用于操作原 App。手柄到达屏幕边缘时继续向外拖一点，提示“松手贴边”后松开即可查看边缘。
