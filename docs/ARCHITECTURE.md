...
`BubbleGesture`负责触点、点击/拖动阈值、四侧安全区和左右贴边；拖出阈值再回原位、多指、取消或长按均不触发点击恢复。菜单使用有边界的FLAG_SECURE窗口，帮助/隐私可阅读，关闭返回进入前状态；未提供的翻译、账号和订阅没有实际操作入口。`CompatUi`统一本地线条图标、颜色、圆角与至少48dp触摸目标。
